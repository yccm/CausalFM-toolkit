# CausalFM for Frontdoor Adjustment Setting
...
class PerFeatureTransformerCATE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        m_encoder: nn.Module | None = None,  # For mediator M
        ...
        **layer_kwargs: Any,
        ...
        if m_encoder is None:
            m_encoder = SequentialEncoder(
                NanHandlingEncoderStep(),
                LinearInputEncoderStep(
                    num_features=2,  # M + nan indicators
                    emsize=DEFAULT_EMSIZE,
                    replace_nan_by_zero=False,
                    bias=True,
                    out_keys=("output",),
        ...
        self.m_encoder = m_encoder  # Mediator encoder
    ...
    def _pack_eval_io(
        ...
        Expected input shapes: 
            X_train: [N_tr, Dx],  A_train: [N_tr, 1],  Y_train: [N_tr, 1], M_train: [N_tr, 1], X_test: [N_te, Dx]
        """
        ...
        M_train = M_train.to(device=device, dtype=dtype)
        ...
        M_all = torch.cat([M_train, torch.full((n_te, M_train.shape[1]), torch.nan, device=device, dtype=dtype)], dim=0)

        # add batch dim = 1 -> [S, 1, D]
        X_all = X_all.unsqueeze(1)
        A_all = A_all.unsqueeze(1)
        Y_all = Y_all.unsqueeze(1)
        M_all = M_all.unsqueeze(1)

        x = {"main": X_all}   
        a = {"main": A_all}   
        y = {"main": Y_all}   
        m = {"main": M_all}   
        return x, a, y, m, single_eval_pos

    @torch.no_grad()
    def estimate_cate_fd(
        self,
        X_train: torch.Tensor,
        A_train: torch.Tensor,
        Y_train: torch.Tensor,
        ...
        **forward_kwargs,
        ...
        """
            inputs: X_train, A_train, Y_train, M_train, X_test 
            returns: dict with predicted CATE on X_test (and optional GMM params)
        """
        x, a, y, m, single_eval_pos = self._pack_eval_io(X_train, A_train, Y_train, M_train, X_test)
        out = self.forward(x, a, y, m, single_eval_pos=single_eval_pos, **forward_kwargs)
        ...
        if return_gmm and ("gmm_pi" in out):
            result.update({
                "gmm_pi":    out["gmm_pi"][0, single_eval_pos:, :],   # [N_te, K]
                "gmm_mu":    out["gmm_mu"][0, single_eval_pos:, :],   # [N_te, K]
                "gmm_sigma": out["gmm_sigma"][0, single_eval_pos:, :],# [N_te, K]
            })
        return result
    ...
    def forward(
        ...
        *,
        ...
        if isinstance(m, dict):
            assert "main" in set(m.keys()), f"Main must be in input keys: {m.keys()}."
        else:
            m = {"main": m}
        _, _, num_m_features = m["main"].shape
        ...
        # Handle m (can be None during inference)
        if m is None:
            m = torch.zeros(
                0,
                batch_size, 
        ...
        m["main"][single_eval_pos:] = torch.nan
        ...
        embedded_m_grouped = embedded_m.unsqueeze(2)  # (b, s, e) -> (b, s, 1, e)
        ...
        embedded_input = torch.cat(
            (embedded_m_grouped, embedded_x, embedded_a_grouped, embedded_y_grouped), dim=2
        )  # (b, s, f_z + f_x + 1 + 1, e)
        ...
        embedded_input = self.add_embeddings_cate(
            embedded_input,
            data_dags=data_dags,
            num_features=num_x_features + num_a_features + num_m_features,
            seq_len=seq_len,
        )
