    ...
    def __init__(self, csv_files, transform=None):
        ...
        for file in self.csv_files:
            ...
            sequence = {
                ...
                'z': torch.FloatTensor(df[z_col].values).unsqueeze(1),  # Shape: (seq_len, 1)
        ...
        if len(self.sequences) > 0:
            ...
            print(f"Number of X features: {self.sequences[0]['X'].shape[1]}")
...
def collate_fn(batch):
    ...
    z = torch.stack([item['z'] for item in batch], dim=1)  # (seq_len, batch_size, 1)
